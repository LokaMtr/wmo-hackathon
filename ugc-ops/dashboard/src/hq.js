(function(){
  ...
  function update(a, dt, t){
    ...
    if(a.headBone && a.look){ const L = a.look;
      if(a.seated && !a.talking){ L.next -= dt; if(L.next <= 0){ L.target = Math.random() < 0.45 ? 0 : (Math.random()-0.5)*0.9; L.next = 3 + Math.random()*7; } }
      else L.target = 0;
      L.yaw += (L.target - L.yaw) * Math.min(1, dt*1.6);
      a.headBone.rotation.y += L.yaw; a.headBone.rotation.x += a.seated ? Math.sin(t*L.speed + L.phase)*0.04 : 0; }
    if(a.target){
